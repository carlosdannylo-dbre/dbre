...
def main() -> None:
    ...
    def process_profile(profile: str) -> Tuple[str, List[ResourceActivity], Optional[str]]:
        ...
        try:
            ...
            profile_activities: List[ResourceActivity] = []
            for activity in (
            ...
            return profile, profile_activities, None
        except Exception as exc:  # boto3/botocore failures
            return profile, [], f"{profile}: {exc}"

    max_workers = min(32, max(4, len(profiles)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_profile, profile): profile for profile in profiles}
        for future in as_completed(futures):
            _, activities, failure = future.result()
            candidates.extend(activities)
            if failure:
